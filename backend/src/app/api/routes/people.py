...
def _svc(db=Depends(get_db)) -> PeopleService:
    """FastAPI dependency that builds a PeopleService bound to the request's DB session."""
    return PeopleService(db)


def _department_name(svc: PeopleService, department_id: uuid.UUID) -> str | None:
...
def _designation_title(svc: PeopleService, designation_id: uuid.UUID) -> str | None:
    """Resolve a designation id to its title, or None if it no longer exists."""
    designation = svc._db.get(Designation, designation_id)
    return designation.title if designation else None


def _manager_name(svc: PeopleService, manager_employee_id: uuid.UUID | None) -> str | None:
    """Resolve a manager's employee id to their display name, or None."""
    if manager_employee_id is None:
        return None
    manager = svc._db.get(Employee, manager_employee_id)
    if manager is None:
        return None
    person = svc._db.get(Person, manager.person_id)
    return f"{person.first_name} {person.last_name}".strip() if person else None


def _employee_out(svc: PeopleService, employee) -> EmployeeOut:
    ...
    return EmployeeOut(
        ...
        department_name=_department_name(svc, employee.department_id),
        designation_id=employee.designation_id,
        designation_title=_designation_title(svc, employee.designation_id),
        manager_employee_id=employee.manager_employee_id,
        manager_name=_manager_name(svc, employee.manager_employee_id),
...
def _designation_out(svc: PeopleService, designation) -> DesignationOut:
    """Build a DesignationOut with its department name attached."""
    return DesignationOut(
        designation_id=designation.designation_id,
        department_id=designation.department_id,
        department_name=_department_name(svc, designation.department_id),
...
def create_department(
    ...
    try:
        return svc.create_department(user, name=body.name)
...
def list_designations(
    ...
    try:
        designations = svc.list_designations(department_id=department_id)
    except NotFoundError_ as err:
        raise HTTPException(status_code=404, detail=str(err)) from err
    return [_designation_out(svc, d) for d in designations]
...
def create_designation(
    ...
    try:
        ...
        raise HTTPException(status_code=409, detail=str(err)) from err
    return _designation_out(svc, designation)
...
def list_employees(
    ...
    return [_employee_out(svc, e) for e in employees]
...
def get_employee(
    ...
    try:
        employee = svc.get_employee(user, employee_id)
    except NotFoundError_ as err:
        raise HTTPException(status_code=404, detail=str(err)) from err
    except PermissionError_ as err:
        raise HTTPException(status_code=403, detail=str(err)) from err
    return _employee_out(svc, employee)
...
def update_employee(
    ...
    try:
        employee = svc.update_employee(user, employee_id, **body.model_dump(exclude_unset=True))
    except NotFoundError_ as err:
        raise HTTPException(status_code=404, detail=str(err)) from err
    except ConflictError_ as err:
        raise HTTPException(status_code=409, detail=str(err)) from err
...
def deactivate_employee(
    ...
    try:
        employee = svc.deactivate_employee(user, employee_id)
    except NotFoundError_ as err:
        raise HTTPException(status_code=404, detail=str(err)) from err
    except ConflictError_ as err:
        raise HTTPException(status_code=409, detail=str(err)) from err
...
def my_profile(
    ...
    return _employee_out(svc, svc.my_profile(user))
