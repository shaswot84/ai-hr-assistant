...
export default function CandidateApplicationDetailPage() {
  ...
  useEffect(() => {
    api
      ...
      .then(setApplication)
      .catch((err) =>
        setError(err instanceof ApiError ? err.detail : "Application not found.")
      );
        ...
        <div className="space-y-6">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-semibold">{application.vacancy_title ?? "Vacancy"}</h1>
          ...
          <div>
            ...
            {application.evaluation ? (
              ...
              <p className="text-sm text-muted">Your resume is still being evaluated — check back shortly.</p>
            )}
          </div>
        </div>
      )}
